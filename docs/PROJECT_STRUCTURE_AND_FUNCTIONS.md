...
| `_skill_context(workspace: Path)` | 所属服务/类型的辅助方法。 |
| `_extract_text(cls, stdout: str)` | 转换、解析或格式化内部数据。 |
| `_extract_event_text(line: str)` | 转换、解析或格式化内部数据。 |
...
| `async run(self, args: Sequence[str], process_key: str, cwd: Path \| str \| None=None, env: dict[str, str] \| None=None, on_stdout_line: Callable[[str], None] \| None=None, stdin_text: str \| None=None)` | 执行 provider、编排流程、事件或外部消息处理。 |
...
| `async _run_windows_powershell_sync(self, command: str, args: Sequence[str], cwd: Path \| str \| None, env: dict[str, str], on_stdout_line: Callable[[str], None] \| None, stdin_text: str \| None)` | 执行 provider、编排流程、事件或外部消息处理。 |
| `async _communicate(process: asyncio.subprocess.Process, on_stdout_line: Callable[[str], None] \| None, stdin_text: str \| None=None)` | 所属服务/类型的辅助方法。 |
...
| `_ensure_claude_skills_link(workspace: Path)` | 校验输入或修复必需的运行状态。 |
...
| Method | 用法 |
|---|---|
| `__init__(self, agents: AgentService, teams: TeamService, queue: QueueService, chat: ChatService, events: EventService, providers: ProviderRegistry, projects: ProjectService \| None=None)` | Python 对象生命周期或协议方法。 |
...
| `_directed_messages_for_member(self, leader_output: str, member_id: str)` | 所属服务/类型的辅助方法。 |
...
| `_split_candidate_ids(raw_ids: str)` | 所属服务/类型的辅助方法。 |
| `_extract_tags(text: str, prefix: str)` | 转换、解析或格式化内部数据。 |
| `_strip_tags(text: str, prefix: str)` | 转换、解析或格式化内部数据。 |
| `_extract_bracket_tags(text: str, prefix: str)` | 转换、解析或格式化内部数据。 |
| `async _run_agent(self, agent: Agent, input_text: str, context: list[str])` | 执行 provider、编排流程、事件或外部消息处理。 |
