...
| `post_chat(team_id: str, payload: ChatMessageCreate, service: ChatService=Depends(get_chat_service))` | 模块级辅助函数；修改前请先查看调用方和测试契约。 |
| `send_chat(team_id: str, payload: ChatMessageCreate, service: ChatService=Depends(get_chat_service), queue: QueueService=Depends(get_queue_service))` | 模块级辅助函数；修改前请先查看调用方和测试契约。 |
...
| `async reset_agent_runtime(agent_id: str, agents: AgentService=Depends(get_agent_service), queue: QueueService=Depends(get_queue_service), orchestrator: Orchestrator=Depends(get_orchestrator))` | 模块级辅助函数；修改前请先查看调用方和测试契约。 |
...
| `dispatch_team_message(team_id: str, payload: TeamDispatchCreate, chat: ChatService=Depends(get_chat_service), orchestrator: Orchestrator=Depends(get_orchestrator))` | 执行 provider、编排流程、事件或外部消息处理。 |
...
| `_migrate_agent_model_provider(conn: sqlite3.Connection)` | 所属服务/类型的辅助方法。 |
| `_migrate_chat_message_client_id(conn: sqlite3.Connection)` | 所属服务/类型的辅助方法。 |
...
| `workspace_args(self, request: ProviderRequest)` | 所属服务/类型的辅助方法。 |
| `async reset_agent(self, agent_id: str)` | 所属服务/类型的辅助方法。 |
...
| Method | 用法 |
|---|---|
| `__init__(self, *, db: Database \| None=None, config_path: str \| Path \| None=None)` | Python 对象生命周期或协议方法。 |
| `setup_workspace(self, workspace: Path)` | 更新或持久化已有资源。 |
| `async run(self, request: ProviderRequest)` | 执行 provider、编排流程、事件或外部消息处理。 |
| `async reset_agent(self, agent_id: str)` | 所属服务/类型的辅助方法。 |
| `_session_key_for(self, agent_id: str, workspace: Path)` | 所属服务/类型的辅助方法。 |
| `_workspace_from_db(self, agent_id: str)` | 所属服务/类型的辅助方法。 |
| `_sync_config_for_agent(self, workspace: Path, agent)` | 所属服务/类型的辅助方法。 |
| `_session_key_path(workspace: Path)` | 所属服务/类型的辅助方法。 |
| `_config_path_for_workspace(workspace: Path)` | 所属服务/类型的辅助方法。 |
| `_load_json_object(path: Path)` | 所属服务/类型的辅助方法。 |
| `_write_json_object(path: Path, value: dict)` | 更新或持久化已有资源。 |
| `_load_sdk()` | 所属服务/类型的辅助方法。 |
| `_safe_model_dump(value)` | 所属服务/类型的辅助方法。 |
| `_tool_call_payload(tool_call)` | 构造 API/兼容层响应或请求载荷。 |
| `_tool_result_payload(tool_result)` | 构造 API/兼容层响应或请求载荷。 |
| `_stringify(value)` | 所属服务/类型的辅助方法。 |
...
| Function | 用法 |
|---|---|
| `_load_builtin_agent_instructions()` | 模块级辅助函数；修改前请先查看调用方和测试契约。 |
...
| `async reset_runtime(self, agent_id: str, *, providers: ProviderRegistry, events: EventService, cleared: dict[str, int] \| None=None)` | 所属服务/类型的辅助方法。 |
| `async run_agent(self, agent: Agent, input_text: str, context: list[str], *, providers: ProviderRegistry, events: EventService, message_id: int \| str \| None=None, session_id: str \| None=None, run_id: str \| None=None, teams: list[Team] \| None=None, project_workspace: Path \| None=None)` | 执行 provider、编排流程、事件或外部消息处理。 |
...
| `build_teammate_block(self, agent_id: str, teams: list[Team])` | 所属服务/类型的辅助方法。 |
| `build_system_prompt(self, agent_id: str, teammates: str='', *, project_workspace: Path \| None=None, config_system_prompt: str \| None=None, config_prompt_file: str \| None=None)` | 所属服务/类型的辅助方法。 |
...
| `_inject_block(prompt: str, start_marker: str, end_marker: str, block: str)` | 所属服务/类型的辅助方法。 |
| `_read_optional_text(path: Path)` | 所属服务/类型的辅助方法。 |
...
| `post(self, team_id: str, payload: ChatMessageCreate, conn: sqlite3.Connection \| None=None, *, emit_event: bool=True)` | 所属服务/类型的辅助方法。 |
| `get(self, message_id: int, conn: sqlite3.Connection \| None=None)` | 读取单个资源、状态或派生视图。 |
| `find_by_client_message_id(self, team_id: str, client_message_id: str, conn: sqlite3.Connection \| None=None)` | 所属服务/类型的辅助方法。 |
| `get_dispatch(self, chat_message_id: int, conn: sqlite3.Connection \| None=None)` | 读取单个资源、状态或派生视图。 |
| `_annotate_dispatch(self, message: ChatMessage, conn: sqlite3.Connection \| None=None)` | 所属服务/类型的辅助方法。 |
| `record_dispatch(self, *, chat_message_id: int, team_id: str, client_message_id: str \| None, queued_count: int, message_ids: list[int], conn: sqlite3.Connection \| None=None)` | 所属服务/类型的辅助方法。 |
| `broadcast_chatroom(self, queue: 'QueueService', team: Team, from_agent: str, content: str, agents: list[Agent], parent: QueueMessage)` | 执行 provider、编排流程、事件或外部消息处理。 |
| `dispatch_team_message(self, queue: 'QueueService', team: Team, message: str, *, sender: str='user', chat_message_id: int \| None=None, conn: sqlite3.Connection \| None=None, emit_event: bool=True)` | 执行 provider、编排流程、事件或外部消息处理。 |
| `post_chatroom_run_outputs(self, team: Team, runs: list)` | 所属服务/类型的辅助方法。 |
| `is_chatroom_origin(message: QueueMessage)` | 所属服务/类型的辅助方法。 |
| `team_child_metadata(parent: QueueMessage \| None, *, team: Team, from_agent: str, kind: str, to_agent: str)` | 所属服务/类型的辅助方法。 |
...
| `_project_workspace_for_message(self, message: QueueMessage)` | 所属服务/类型的辅助方法。 |
| `async reset_agent_session(self, agent_id: str, *, cleared: dict[str, int] \| None=None)` | 所属服务/类型的辅助方法。 |
...
| `_summarize_workflow_output(text: str, max_length: int=240)` | 所属服务/类型的辅助方法。 |
...
| `dispatch_team_message(self, team_id: str, message: str, *, sender: str='user', chat_message_id: int \| None=None, conn: Any \| None=None, emit_event: bool=True)` | 执行 provider、编排流程、事件或外部消息处理。 |
| `async _run_agent(self, agent: Agent, input_text: str, context: list[str], *, message_id: int \| str \| None=None, session_id: str \| None=None, run_id: str \| None=None, teams: list[Team] \| None=None, project_workspace: Path \| None=None)` | 执行 provider、编排流程、事件或外部消息处理。 |
...
| `enqueue(self, payload: MessageCreate, conn: sqlite3.Connection \| None=None, *, emit_event: bool=True)` | 队列、响应或消息流转操作。 |
...
| `_build_in_model_env_settings()` | 所属服务/类型的辅助方法。 |
| `_sync_build_in_model_env(build_in_model: dict[str, Any])` | 所属服务/类型的辅助方法。 |
...
| `for_agent(self, agent_id: str)` | 所属服务/类型的辅助方法。 |
| `order_agents_for_team(team: Team, agents: list[Agent])` | 所属服务/类型的辅助方法。 |
| `resolve_team_context_for_agent(agent_id: str, teams: list[Team])` | 所属服务/类型的辅助方法。 |
| `resolve_team_for_tag(team_id: str, teams: list[Team], agent_id: str)` | 所属服务/类型的辅助方法。 |
| `agent_lookup(agents: list[Agent])` | 所属服务/类型的辅助方法。 |
| `mentions_from_runs(self, runs: list[AgentRun], agents: list[Agent])` | 所属服务/类型的辅助方法。 |
| `member_chain_input(self, team: Team, original_request: str, leader_run: AgentRun, previous_member_runs: list[AgentRun], member_id: str)` | 所属服务/类型的辅助方法。 |
| `leader_summary_input(self, team: Team, original_request: str, leader_run: AgentRun, member_runs: list[AgentRun])` | 所属服务/类型的辅助方法。 |
| `format_runs(runs: list[AgentRun])` | 转换、解析或格式化内部数据。 |
...
| `merge_workflow_memory(left: dict[str, Any], right: dict[str, Any])` | 模块级辅助函数；修改前请先查看调用方和测试契约。 |
...
| Method | 用法 |
|---|---|
| `__init__(self, db: Database, teams: TeamService, settings_service: SettingsService \| None=None)` | Python 对象生命周期或协议方法。 |
...
| `async run_workflow(self, message: QueueMessage, team: Team, agents: list[Agent], workflow: TeamWorkflow, *, run_agent: Callable[..., Awaitable[AgentRun]], queue: QueueService, chat: ChatService, events: EventService, project_workspace_for_message: Callable[[QueueMessage], Path \| None])` | 执行 provider、编排流程、事件或外部消息处理。 |
| `workflow_node_input(team: Team, workflow_id: str, original_request: str, memory: WorkflowMemoryState, node, node_name: str, predecessor_ids: list[str], outputs: dict[str, str])` | 所属服务/类型的辅助方法。 |
| `format_workflow_predecessors(cls, predecessor_ids: list[str], outputs: dict[str, str])` | 转换、解析或格式化内部数据。 |
| `summarize_workflow_output(self, text: str, max_length: int=240)` | 所属服务/类型的辅助方法。 |
| `format_recent_workflow_outputs(recent_outputs: list[dict[str, str]])` | 转换、解析或格式化内部数据。 |
| `_initial_workflow_memory()` | 所属服务/类型的辅助方法。 |
| `_workflow_memory_after_run(self, memory: WorkflowMemoryState, node_id: str, output: str)` | 所属服务/类型的辅助方法。 |
| `_compact_workflow_memory(self, memory: WorkflowMemoryState)` | 所属服务/类型的辅助方法。 |
| `_get_workflow_summary_chain(self)` | 读取单个资源、状态或派生视图。 |
| `_build_in_model_setting(env_name: str, legacy_env_name: str, default: str='')` | 所属服务/类型的辅助方法。 |
| `_build_in_model_number(env_name: str, legacy_env_name: str, default: float)` | 所属服务/类型的辅助方法。 |
| `_normalize_summary_text(text: str, max_length: int)` | 转换、解析或格式化内部数据。 |
| `_langchain_runnable_for_agent(agent: Agent, run_agent: Callable[..., Awaitable[AgentRun]])` | 所属服务/类型的辅助方法。 |
| `_build_langgraph_workflow(self, *, team: Team, workflow_id: str, message: QueueMessage, agents: list[Agent], node_by_id: dict[str, Any], agent_by_id: dict[str, Agent], predecessors: dict[str, list[str]], outgoing: dict[str, list[str]], edge_pairs: list[tuple[str, str]], conditional_edges: list[Any], entrypoint: str, run_agent: Callable[..., Awaitable[AgentRun]], queue: QueueService, events: EventService, project_workspace_for_message: Callable[[QueueMessage], Path \| None])` | 所属服务/类型的辅助方法。 |
| `compile_workflow_routing_function(node)` | 所属服务/类型的辅助方法。 |
| `route_from_output(output: str, conditions: list[str])` | 所属服务/类型的辅助方法。 |
